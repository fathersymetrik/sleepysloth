(function(global,factory){typeof exports==='object'&&typeof module!=='undefined'?module.exports=factory(require('jquery')):typeof define==='function'&&define.amd?define(['jquery'],factory):(global.Mark=factory(global.jQuery))}(this,(function($){'use strict';$=$&&$.hasOwnProperty('default')?$['default']:$;class DOMIterator{constructor(ctx,iframes=!0,exclude=[],iframesTimeout=5000){this.ctx=ctx;this.iframes=iframes;this.exclude=exclude;this.iframesTimeout=iframesTimeout}
static matches(element,selector){const selectors=typeof selector==='string'?[selector]:selector,fn=(element.matches||element.matchesSelector||element.msMatchesSelector||element.mozMatchesSelector||element.oMatchesSelector||element.webkitMatchesSelector);if(fn){let match=!1;selectors.every(sel=>{if(fn.call(element,sel)){match=!0;return!1}
return!0});return match}else{return!1}}
...
observeIframeLoad(ifr,successFn,errorFn){let called=!1,tout=null;const listener=()=>{if(called){return}
called=!0;clearTimeout(tout);try{if(!this.isIframeBlank(ifr)){ifr.removeEventListener('load',listener);this.getIframeContents(ifr,successFn,errorFn)}}catch(e){errorFn()}};ifr.addEventListener('load',listener);tout=setTimeout(listener,this.iframesTimeout)}
...
waitForIframes(ctx,done){let eachCalled=0;this.forEachIframe(ctx,()=>!0,ifr=>{eachCalled++;this.waitForIframes(ifr.querySelector('html'),()=>{if(!(--eachCalled)){done()}})},handled=>{if(!handled){done()}})}
...
checkEnd()},checkEnd)}})}
createIterator(ctx,whatToShow,filter){return document.createNodeIterator(ctx,whatToShow,filter,!1)}
createInstanceOnIframe(contents){return new DOMIterator(contents.querySelector('html'),this.iframes)}
compareNodeIframe(node,prevNode,ifr){const compCurr=node.compareDocumentPosition(ifr),prev=Node.DOCUMENT_POSITION_PRECEDING;if(compCurr&prev){if(prevNode!==null){const compPrev=prevNode.compareDocumentPosition(ifr),after=Node.DOCUMENT_POSITION_FOLLOWING;if(compPrev&after){return!0}}else{return!0}}
return!1}
...
checkIframeFilter(node,prevNode,currIfr,ifr){let key=!1,handled=!1;ifr.forEach((ifrDict,i)=>{if(ifrDict.val===currIfr){key=i;handled=ifrDict.handled}});if(this.compareNodeIframe(node,prevNode,currIfr)){if(key===!1&&!handled){ifr.push({val:currIfr,handled:!0})}else if(key!==!1&&!handled){ifr[key].handled=!0}
return!0}
...
class RegExpCreator{constructor(options){this.opt=Object.assign({},{'diacritics':!0,'synonyms':{},'accuracy':'partially','caseSensitive':!1,'ignoreJoiners':!1,'ignorePunctuation':[],'wildcards':'disabled'},options)}
create(str){if(this.opt.wildcards!=='disabled'){str=this.setupWildcardsRegExp(str)}
...
str=this.createAccuracyRegExp(str);return new RegExp(str,`gm${this.opt.caseSensitive ? '' : 'i'}`)}
sortByLength(arry){return arry.sort((a,b)=>a.length===b.length?(a>b?1:-1):b.length-a.length)}
escapeStr(str){return str.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,'\\$&')}
createSynonymsRegExp(str){const syn=this.opt.synonyms,sens=this.opt.caseSensitive?'':'i',joinerPlaceholder=this.opt.ignoreJoiners||this.opt.ignorePunctuation.length?'\u0000':'';for(let index in syn){if(syn.hasOwnProperty(index)){let keys=Array.isArray(syn[index])?syn[index]:[syn[index]];keys.unshift(index);keys=this.sortByLength(keys).map(key=>{if(this.opt.wildcards!=='disabled'){key=this.setupWildcardsRegExp(key)}
key=this.escapeStr(key);return key}).filter(k=>k!=='');if(keys.length>1){str=str.replace(new RegExp(`(${keys.map(k => this.escapeStr(k)).join('|')})`,`gm${sens}`),joinerPlaceholder+`(${keys.map(k => this.processSynonyms(k)).join('|')})`+joinerPlaceholder)}}}
return str}
processSynonyms(str){if(this.opt.ignoreJoiners||this.opt.ignorePunctuation.length){str=this.setupIgnoreJoinersRegExp(str)}
return str}
...
return!0})});return str}
createMergedBlanksRegExp(str){return str.replace(/[\s]+/gmi,'[\\s]+')}
...
class Mark{constructor(ctx){this.ctx=ctx;this.ie=!1;const ua=window.navigator.userAgent;if(ua.indexOf('MSIE')>-1||ua.indexOf('Trident')>-1){this.ie=!0}}
set opt(val){this._opt=Object.assign({},{'element':'','className':'','exclude':[],'iframes':!1,'iframesTimeout':5000,'separateWordSearch':!0,'acrossElements':!1,'ignoreGroups':0,'each':()=>{},'noMatch':()=>{},'filter':()=>!0,'done':()=>{},'debug':!1,'log':window.console},val)}
...
wrapRangeInMappedTextNode(dict,start,end,filterCb,eachCb){dict.nodes.every((n,i)=>{const sibl=dict.nodes[i+1];if(typeof sibl==='undefined'||sibl.start>start){if(!filterCb(n.node)){return!1}
...
return!0})}
wrapGroups(node,pos,len,eachCb){node=this.wrapRangeInTextNode(node,pos,pos+len);eachCb(node.previousSibling);return node}
separateGroups(node,match,matchIdx,filterCb,eachCb){let matchLen=match.length;for(let i=1;i<matchLen;i++){let pos=node.textContent.indexOf(match[i]);if(match[i]&&pos>-1&&filterCb(match[i],node)){node=this.wrapGroups(node,pos,match[i].length,eachCb)}}
return node}
wrapMatches(regex,ignoreGroups,filterCb,eachCb,endCb){const matchIdx=ignoreGroups===0?0:ignoreGroups+1;this.getTextNodes(dict=>{dict.nodes.forEach(node=>{node=node.node;let match;while((match=regex.exec(node.textContent))!==null&&match[matchIdx]!==''){if(this.opt.separateGroups){node=this.separateGroups(node,match,matchIdx,filterCb,eachCb)}else{if(!filterCb(match[matchIdx],node)){continue}
...
mark(sv,opt){this.opt=opt;let totalMatches=0,fn='wrapMatches';const{keywords:kwArr,length:kwArrLen}=this.getSeparatedKeywords(typeof sv==='string'?[sv]:sv),handler=kw=>{const regex=new RegExpCreator(this.opt).create(kw);let matches=0;this.log(`Searching with expression "${regex}"`);this[fn](regex,1,(term,node)=>{return this.opt.filter(node,kw,totalMatches,matches)},element=>{matches++;totalMatches++;this.opt.each(element)},()=>{if(matches===0){this.opt.noMatch(kw)}
...
$.fn.mark=function(sv,opt){new Mark(this.get()).mark(sv,opt);return this};$.fn.markRegExp=function(regexp,opt){new Mark(this.get()).markRegExp(regexp,opt);return this};$.fn.markRanges=function(ranges,opt){new Mark(this.get()).markRanges(ranges,opt);return this};$.fn.unmark=function(opt){new Mark(this.get()).unmark(opt);return this};return $})))
